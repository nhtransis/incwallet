...
// unstakingCmd represents the unstaking command
...
to quickly create a Cobra application.`,
	Run: func(cmd *cobra.Command, args []string) {
		if len(args) != 0 {
			...
			UnStaking(tool, privateKeys[0], privateSeeds[0])
		}
	},
}
...
func init() {
	...
	// unstakingCmd.PersistentFlags().String("foo", "", "A help for foo")

	// Cobra supports local flags which will only run when this command
	// is called directly, e.g.:
	// unstakingCmd.Flags().BoolP("toggle", "t", false, "Help message for toggle")
}
